...
$\sim$240-1640 days.  Where observed, the profile of the
Ly$\alpha$/\nv emission complex also changes, and there is tentative
evidence for changes in the \mgii line.  Although line measurements
from the three quasars show large changes in the \civ\ flux-line width
plane, the quasars are not seen to be outliers when considered against
the full $z\sim2$ quasar population in terms of (rest) Equivalent
Width and FWHM properties.  We put these observations in context with
...
3.4$\mu$m and WISE W2 at 4.6$\mu$m, respectively \citep{Cutri2011}. We
...
Object    & MJD  &      $\nu L_{\nu}$    &         Slope                             &   Luminosity         &     FWHM                 &    V$_{\rm off}$    & EW                                 & log($\nu L_{\nu}^{0.5} \times {\rm FWHM}^2$) \\
 ...
 J2228+2201 & 56960   & 7842 $\pm$ 25     &  -1.72 $\pm$ 0.02 & 301.0  $\pm$ 7.2  &  7136 $\pm$  210   & -276 $\pm$  64   &  61.99 $\pm$  1.48  &    9.65 $\pm$ 0.03\\ 
    ...
    \caption{Continuum at 1450\AA\ and \civ\ spectral measurements for the
      ...
      calculated by QSFit.  The last column shows the virial product
      calculated as $\nu L_{\nu}^{0.5} \times {\rm FWHM}^2$.  $^{*}$The
      \civ\ line is very faint (with respect to the continuum), and the
      associated estimates are likely unreliable.}
...
FWHM and EW are given in Tab.~\ref{tab:QSFit-results}. All fits are
...
This allows us to account for the narrow peak of the \civ\ line. 
No narrow components are considered for several reasons: in the epochs
of highest brightness the ``narrow'' component would have FWHM
$\sim2-3 \times 10^{3}$ km s$^{-1}$, i.e. values exceeding the usual
widths of genuine narrow lines ($\lesssim 10^3$ km s$^{-1}$); by
allowing a second component, to have such large widths their
parameters would become highly degenerate with the ``broad''
components, and the latter would also have much larger FWHM
($\sim10^{4}$ km s$^{-1}$); and by neglecting the narrow components we
have a consistent fit across all epochs.
...
variations in the line luminosities.  The FWHM of broad lines is
likely related to the mass of the supermassive black hole powering the
quasar phenomenon, which is supposed to be constant on any human
timescale.  Hence it is instructive to check whether the virial
product, which is the basic quantity used to calculate the single
epoch mass estimate, show any variation.  The virial product,
calculated as $\nu L_{\nu}^{0.5} \times {\rm FWHM}^2$, is reported in
the last column of Tab.~\ref{tab:QSFit-results}.  The uncertainty
typically associated to the single epoch mass estimate is $\sim$0.5
dex, hence the virial product at all epochs are remarkably constant
and compatible with a single value of black hole mass for each source,
even in those cases where the \civ\ estimates are possibly
unreliable. The object showing larger variation is J2228+2201,
although the extreme values span a range of 0.48 dex.

Finally, also the velocity offsets of the \civ\ line are also
...
\begin{figure*}
  \centering
 \includegraphics[width=\textwidth, trim={0cm 0 0 0},clip]{figures/CIVregions.png}
%  \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/spec-2089-53498-0427.png}
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/J1205p3422_58538.png}
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/J1205p3422_58693.png}\\
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/spec-2948-54553-0614.png}
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/spec-5201-55832-0178_v5_10_0.png}
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/J1638p2827_58583.png}\\
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/spec-6118-56189-0720.png}
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/spec-7582-56960-0790.png}
 % \includegraphics[width=.3\textwidth, trim={0cm 0 0 0},clip]{figures/J2228p2201_58693.png}
...
\begin{figure}
  ...
  {figures/CIV_CLQs_Baldwin_20191129.png}
   \vspace{-12pt}
...
Figure~\ref{fig:CIV_Baldwin}. This figure effectively reproduces Figure~15 from \citet{Calderone2017}, but where we now
add the measurements from the 3 \civ\ CLQ quasars at each epoch. 
...
with that from \citet[][ $\beta=-0.23$]{Hamann2017}.
...
\end{figure}
\section{Discussion}
\subsection{Continuum and Line Changes}
...
there is (at least in some cases) a relationship between the continuum
...
However, there is no evidence for broad UV emission lines, including
\civ\ , \ciii\ , or \mgii.  The authors attribute the lack of broad UV
...
probed by the SDSS and Pan-STARRS1 surveys.  For our adopted
signal-to-noise ratio threshold on variability of broad H$\beta$
emission, they find 17 new CLQs (which is $\sim20$ of the observerd
sample). These candidates are at lower Eddington ratio relative to the
overall quasar population, while the CLQ fraction increases from 10\%
to roughly half, as the continuum flux ratio between repeat spectra at
3420 \AA\ (rest-frame) increases from 1.5 to 6.  \citet{Homan2019}
investigate the responsiveness of the \mgii broad emission line
doublet in AGN on timescales of several years.  This study is based on
a the same extremely variable quasar sample as was used for
\citet{MacLeod2019}.  By again focussing on quasars that show large
...
%Lower redshift $z<0.9$ CLQs have traditionally been identified via large changes in the  H$\beta$ emission line. H$\beta$ emission is associated with a 2.55 eV energy difference.  Thus we can place the high-$z$, HIL \civ\ in context of Balmer H$\beta$ CLQs at lower-$z$.

The \civ\ line is one of the strongest collisionally excited lines in quasar
spectra \citep[e.g.][]{HamannFerland1999}, and 
\civ\ emission probes the photoionization environment produced by the innermost
disk, as indicated by RM time-delay measurements.
%%
In standard \citet{SS73} 
...
variable quasars explored by \citet{Dyer2019}, and similar to those
...
\begin{figure}
  ...
  {figures/CIV_CLQs_MBHvsREW_QSFit20K.png}
   \vspace{-12pt}
   \caption[]{The    \civ\ Equivalent width and the virial black hole masses.}
  \label{fig:CIV_MBHvsREW}
\end{figure}

Figure~\ref{fig:CIV_MBHvsREW} shows the  \civ\ Equivalent width and the virial black hole masses.
...
By comparing the observed correlations between the UV-to-X-ray
spectral index ($\alpha_{\rm OX}$) and Eddington ratio in AGN to those
predicted from observations of X-ray binary outbursts,
\citet{Ruan2019a} find %that the observed correlations between the
($\alpha_{\rm OX}$ and Eddington ratio of that AGN display a
remarkable similarity to accretion state transitions in prototypical
...
%% We investigate this reporting the Eddington ratios of the three quasars in Table~\ref{tab:Eddington_ratios}.

{\bf Must say something about these objects all being $>10^{9}$ but having
the quick \civ\ line changes.}
