...
their first optical identifications
\citep{MatthewsSandage1963}. %MacLeod2012}.  However, dramatic changes
in the broad emission lines (BELs) of quasars was only recently first
identified \citep{LaMassa2015}.  Samples of over 100 ``changing-look''
quasars (CLQs) or ``changing-state'' quasars (CSQs) have now been
assembled \citep[e.g.,][]{MacLeod2019, Graham2019b}. The community
uses both these terms as a cover for the underlying physics. For sake
of argument, CLQs can potentially be thought of as the extension to
the BELs of quasar continuum variability \citep[e.g., ][]{MacLeod2012}
...
\begin{table*}
  \begin{centering}
    ...
    \caption{
      ...
      to create a \cv\ ion.  Transition level configurations are given in
...
required for doubly-ionised C\,{\sc iii} to become triply-ionised
\civ, and 64.49 eV (4.74 Ry) is needed to ionise \civ\ itself. This
...
between the \civ\ equivalent width (EW) and luminosity (i.e., the
...
V_{\rm off}\rangle_{\rm RQ} \sim$810 km s$^{-1}$ for radio-quiet (RQ)
quasars and $\langle V_{\rm off}\rangle_{\rm RL} \sim$360 km s$^{-1}$
...
is somewhat interesting, the main value of this study is to move from
...
Graham2017, Graham2019b, Stern2017, Stern2018, Ross2018}.
...
sky north of $-$30 declination every night. ZTF observes the sky in
...
\begin{figure*}
  ...
  {figures/J1205+3422_landscape_20191211.png}
  ...
  \vspace{-12pt}
  \caption[]{The three high-redshift CLQ quasars; 
    J1205+3422 (top), 
    J1638+2827 (middle) and  
    J2228+2201 (bottom). 
    The light curve data are presented in the panels on the left hand side, with the timing of the
    spectral observations indicated with vertical dashed lines. 
...
This allowed us to predict the photometric magnitudes at the epochs of
the SDSS spectra for each source.  Those with $|\Delta V| > 0.3$ were
then selected for visual inspection, looking for at least a 50\%
change in magnitude between the two epochs.  Only three quasars, SDSS
...
Figure~\ref{fig:civ_clqs} presents the optical and mid-infrared light
curves for three high-redshift CLQ quasars.  Figure~\ref{fig:civ_clqs}
...
\ciii, and \mgii. \ciii\ and \civ\ have large blueshifts,
$\approx$2600$\pm$150 and $\approx$1150$\pm$100 km s$^{-1}$,
...
epoch spectrum, \civ\ is broad and bright, as is \ciii. However,
$\approx$400 rest-frame days later, the broad \civ\ and \ciii\
emission lines have faded, the continuum slope around 1400\AA\ has
changed from $\approx-1.48$ to $\approx-2.25$, but the \lya/\nv
emission complex is very similar in shape and line flux
intensity. Around 870 days in the rest-frame after the second spectral
epoch, at the time of the third spectral epoch, the source has
brightened from optical magnitudes of $\sim 20$ mag to $\sim 18.5$
mag. Ly$\alpha$, N\,{\sc v}, \civ, \ciii, and \mgii\ are all apparent
and broad, with \mgii\ being seen for the first time at high
signal-to-noise. An absorption feature between \lya and \nv is seen in
all three spectral epochs.
...
rest-frame days, \civ\ and \ciii\ both {\it emerge} as BELs and the
standard UV/blue continuum slope increases in flux.  Then, over the
...
Table~\ref{tab:QSFit-results}. All fits are performed with $E(B-V) =
0$ and the best fit models in the region of the \civ\ emission line
are shown in Figure~\ref{fig:QSFit-CIV}.
...
\begin{figure}
  ...
  {figures/CIV_CLQs_MBHvsREW_20191211.png}
   \vspace{-12pt}
   \caption[]{
...
The uncertainty typically associated with single-epoch mass estimates
is $\sim 0.5$~dex, implying that the virial products for our three
...
\begin{figure}
  \centering
  \includegraphics[width=8.5cm, trim=0.2cm 0.2cm 0.0cm 0.2cm, clip]
  {figures/CIV_CLQs_REWvsFWHM_20191211.png}
  \vspace{-12pt}
...
\begin{figure*}
  ...
  \caption{Temporal evolution of the spectral properties of the three
    quasars considered in this work.}
...
\begin{table}
  ...
  \begin{tabular}{l l l r}
    ...
    \hline
                     & 53498 & 47.216     &  $-$0.610   \\
 J1205+3422 & 58538 &  46.070    &   $-$1.756  \\   %% L_Edd (J12)  = 47.82637   for log(M) = 9.726
                     & 58693 &  46.567    &  $-$1.259  \\                    
    \hline 
                     & 54553 & 46.166   & $-$1.025  \\
...
\begin{figure}
  \centering
  \includegraphics[width=8.5cm, trim=0.2cm 0.2cm 0.0cm 0.2cm, clip]
  {figures/CIV_CLQs_Baldwin_20191211.png}
   \vspace{-12pt}
   \caption[]{\civ\ equivalent width versus underlying continuum luminosity,
     ...
     Figure~\ref{fig:CIV_MBHvsREW}.  The dashed red line has slope
     $\beta=-0.38$.}
...
is $-0.20$, consistent with \citet[][ $\beta=-0.25$, but for
bolometric luminosity rather than rest-frame UV continuum luminosity]
...
Subject to small number statistics, the slope of the intrinsic Baldwin
effect for these three high-redshift \civ\ CLQs is somewhat steeper
than the population trends, with $\beta \approx-0.38$, shown by the
dashed red line in Figure~\ref{fig:CIV_Baldwin}.
...
Using the same dataset as \citet{MacLeod2019}, \citet{Homan2019}
...
\citet{Homan2019} find that \mgii\ clearly does respond to the
continuum.  However, a key finding from \citet{Homan2019} is that the
...
\citet{Graham2019b} report the largest number of H$\beta$ CLQs to
date, with a sample of 111 sources identified. \citet{Graham2019b}
...
quasar spectra \citep[e.g.,][]{HamannFerland1999}, and \civ\ emission
probes the photoionisation environment produced by the innermost disk,
...
an intrinsic Baldwin effect (see Figure~\ref{fig:CIV_Baldwin}). That
is, the excitation of the \civ\ line is driven by the changes in the
continuum; however, the excited gas cannot reach an equilibrium
excitation state as quickly as the continuum changes, making the slope
of the intrinsic Baldwin effect greater than that of the overall
ensemble slope, which is derived from many single-epoch observations,
the majority of which are assumed to be in equilibrium.
...
\begin{figure*}
  ...
  {figures/MJD_vs_Eddington_20191211.png}
  \vspace{-12pt}
...
However, one thing to note is that radiation pressure should be much
more important in AGNs than in stellar mass black hole systems,
implying that sound speed is much faster than expected from the gas
temperature alone.  In Galactic binaries spectral hardening
...
We thank Andy Lawrence, Mike Hawkins and David Homan for useful discussion.
