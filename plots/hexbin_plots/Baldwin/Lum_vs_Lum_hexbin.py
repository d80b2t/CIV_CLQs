...
## AXES LIMITS 
xmin =  43.4                     ## 43.4 for L_1350, L1450; 44.4 for LBol
xmax =  log_Lum1450.max()        ## log_LBol.max() - 0.2
ymin =  xmin                     ## -1.0 when x = LBol.
ymax =  xmax                     ## -1.0 when x = LBol.
...
ax.set_xlabel(r"log$_{10}$(L$_{\rm 1450}$) [ergs/s] ", fontsize=fontsize)
ax.set_ylabel(r"log$_{10}$(L$_{\rm 1350}$) [ergs/s] ", fontsize=fontsize)


## AXES TICK FORMAT
ax.tick_params(axis='both', which='major', labelsize=labelsize, bottom=True, top=True, left=True, right=True)
ax.tick_params(axis='both', which='minor', labelsize=labelsize, bottom=True, top=True, left=True, right=True)

#ax.xaxis.set_ticks(np.arange(44, 48, 1.0))
#ax.yaxis.set_ticks(np.arange(42, 49, 1.0))
#ax.yaxis.set_ticks(np.arange(44, 48, 1.0))
...
#boss          = mlines.Line2D([], [], label='229,004 BOSS quasars', color='skyblue',
                       marker=".", markeredgecolor='k', markeredgewidth=1.4, markersize=7,  linestyle='None')
bestfit_line  = mlines.Line2D([], [], label=r'$\beta = $'+slope_str+',   c='+intercept_str, color='r')

handles=[boss, bestfit_line ]
leg = ax.legend(loc='lower right', fontsize=fontsize/1.25, handles=handles, 
                frameon=True, framealpha=1.0, fancybox=True)

## SAVING THE FIGURE
plt.savefig('CIV_CLQs_Lum_vs_Lum_temp.png', format='png')
plt.close()
